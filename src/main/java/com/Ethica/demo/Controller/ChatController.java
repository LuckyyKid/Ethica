    ...
    public ResponseEntity<Map<String, String>> chat(
        ...
        if (currentUser == null) {
            return ResponseEntity.status(401)
                    .body(Map.of(RESPONSE_KEY, "User is not connected."));
        }

        ClientPortfolio portfolio =
                portfolioRepository.findByUser(currentUser).orElse(null);

        if (portfolio == null) {
            return ResponseEntity.status(404)
                    .body(Map.of(RESPONSE_KEY, "No portfolio found."));
        }
        ...
        if (trades.isEmpty()) {
            return ResponseEntity.ok(
                    Map.of(RESPONSE_KEY, "No transactions found to analyze the portfolio.")
            );
        }

        StringBuilder portfolioSummary = new StringBuilder();
        for (Trade trade : trades) {
            portfolioSummary.append(trade.isBuy() ? "Bought" : "Sold")
                    .append(" ")
        ...
        String aiResponse =
                aiService.getMarketSummary(userQuestion, portfolioSummary.toString());

        return ResponseEntity.ok(Map.of(RESPONSE_KEY, aiResponse));
    }
}
